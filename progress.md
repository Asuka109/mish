...
| CUT-03 Query/Store/UI state | active `/root/cut03_query_store_ui`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut03/mihomo-web-client`; branch `codex/cut03-query-store-ui`; cumulative source `77633778`, implementation baseline `da7c8237`; `gpt-5.6-luna/max`, Chinese, confirmation-only | CUT-01 and CUT-02 integrated on cumulative branch | Exclusive scope: new `packages/ui-state/**`, `apps/web/src/**` UI/data projection files and tests, plus exact packet dependency updates to cumulative root `pnpm-lock.yaml` only; pin `@tanstack/query-core`/`@tanstack/react-query` `5.101.4`, `@orpc/tanstack-query` `1.15.0`, and core-only `@tanstack/store` `0.11.1`; remote snapshots/freshness/retry/invalidation belong only to Query, lifecycle only to XState actors, Event Iterator chunks only to Query cache or actors, UI-only cross-component state only to the minimal `@mish/ui-state` wrapper, and single-component transients remain React local state; prove stale/reconnect, cancellation, unmount/remount, accessibility, production isolation, and no RN/shared DOM adapter; no host shell/effects, Zag, second machine runtime, compatibility/fallback/dual-write, POC imports, Rust parity, direct `dev` merge, release, or real network/VPN/TUN/Core effect; PR target only `codex/typescript-cutover` |
