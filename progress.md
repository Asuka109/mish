...
| CUT-02b Domain ESM consumer boundary | delivered through commit `f5dbc4f0` / PR #556 on existing `/root/cut02_domain_actors`; exact 13-file domain-source scope; coordinator domain typecheck/tests 17, Web consumer typecheck, diff/specifier/scope review passed; worker frozen install, P2 tests 10/typecheck, oRPC 16, legacy RPC 25, admission, format/lint passed; Fast PR 6m57s and Android platform 6m13s passed, Android lifecycle pending; not yet integrated | CUT-03 Web typecheck proved public domain source `.ts` specifiers require a forbidden consumer tsconfig relaxation | Mechanically replace only relative `packages/domain/src/**` `.ts` import/export specifiers with ESM `.js` specifiers, preserve all public types/runtime semantics, and prove domain/P2/Web consumer typechecks without `allowImportingTsExtensions`; system-boundary not applicable; no package/lock/Web tsconfig/API/logic/host effect, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
| CUT-03 Query/Store/UI state | paused with uncommitted WIP on existing `/root/cut03_query_store_ui`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut03/mihomo-web-client`; branch/PR `codex/cut03-query-store-ui` / #553; coordinator ui-state 20/domain 17/oRPC 16 passed, but Web typecheck exposed CUT-02b plus the analogous in-scope ui-state `.js` specifier fix; no workaround committed | CUT-01, CUT-02b, and P6g | After CUT-02b, finish the real actor/oRPC/Query/Store composition and convert ui-state relative source specifiers to ESM `.js` within CUT-03 scope; never relax Web tsconfig. Preserve one session actor, Query-only remote cache, Store-only presentation, no second Events consumer, helper shell, fallback/dual-write, host effect, direct `dev` merge, release, or real network/VPN/TUN/Core effect; prove stale/reconnect, bounded retry/chunks, cleanup/cancellation, StrictMode remount, accessibility, and production graph |
