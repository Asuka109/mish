...
| P6g CUT-03 Web importer correction | active `/root/p6_cutover_admission`; worktree `/Users/asuka/.codex/worktrees/wave3c-p6g/mihomo-web-client`; branch `codex/p6g-cut03-web-importer`; source `dev@be3f5f8f`; `gpt-5.6-luna/max`, Chinese, confirmation-only | CUT-03 delivery exposed an internally inconsistent scope | Amend only `docs/architecture/typescript-cutover-admission.md`, `scripts/check-typescript-cutover-admission.ts`, and its test so CUT-03 may update `apps/web/package.json` only for exact `@mish/ui-state`, `@mish/orpc-client`, and `@mish/domain` workspace dependencies plus its already-authorized cumulative lock importer; retain CUT-06 ownership of root manifests/tsconfig/CI cleanup, reject every other manifest/path expansion, and do not change the selected architecture, versions, one-shot barrier, authority envelope, or no-real-effect policy |
...
| CUT-03 Query/Store/UI state | delivered foundation commit `da9834f7` / PR #553 by `/root/cut03_query_store_ui`; not accepted or merge-authorized pending P6g and real Web consumer follow-up; isolated package frozen install, typecheck/tests 18, oRPC 16, domain 15, Web typecheck, admission, format/lint, and production isolation passed; exact scope prevented adding the Web importer, and the Worker correctly removed helper-only/relative-source wiring instead of claiming migration | CUT-01 and CUT-02 integrated on cumulative branch; P6g now required | `@mish/ui-state` pins the admitted Query/Store/oRPC versions, enforces retry `0..3`, routes Event Iterator only to Query/XState, and proves Store/RN-Hermes/no-DOM boundaries; after P6g, the same Worker must add the exact Web workspace dependencies and real Query/XState consumer composition under `apps/web/src/**`, prove stale/reconnect/cancellation/unmount-remount/accessibility, and retain all no-host-effect/no-Zag/no-compat/no-dual-write/no-direct-`dev` constraints before PR #553 can be accepted |
