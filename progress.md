...
| CUT-12 Corrected product-UI DMG handoff | accepted from cumulative source `codex/typescript-cutover@fa97abea` after PR #565 / fixture fix `78701a26`; retained under ignored `.scratch/deliverables/typescript-cutover-fa97abea`. DMG SHA-256 `6a5cd301…175a9e29`, 109,505,327 bytes, with `SHA256SUMS` and `VERIFICATION.md` | CUT-11A through CUT-11C integrated | Restored product UI DMG passed installed-copy default persistence 1/1 and explicit auto-quit 2/2 with bounded renderer/product/status/transcript, owned-process cleanup, security/privacy, source-clean, and valid DMG checksum evidence. The fixture now mounts, verifies, copies to a private writable installation root, detaches only its task mount, and launches the installed copy instead of executing Electron from the read-only image. Product, Electron, and RN CI passed. Earlier `a9c97c02`/`c1b78fc9` DMGs are superseded. User mounts remained untouched; no sign/notarize/publish/deploy, external write, real network/VPN/TUN/Core/system effect, or `dev` implementation merge |
