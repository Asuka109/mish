...
| CUT-05 RN host/native seam | active as `/root/cut05_rn_host`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut05/mihomo-web-client`; branch `codex/cut05-rn-host`; cumulative ledger source `codex/typescript-cutover@993756e2`, implementation baseline `a30c59ec`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance, final-only escalation | CUT-01, CUT-02, CUT-03, and CUT-04 integrated | Exclusive scope is new RN host files under `apps/mobile/**` excluding every `src-tauri/**`, Kotlin/Swift/Objective-C adapters only for irreducible host effects, transcript/replay tests, and exact cumulative root lock updates. Deliver RN 0.87.0 New Architecture/Hermes composition with contracts/oRPC, XState actors, Query core, and framework-agnostic Store; dual-ABI `arm64-v8a,x86_64` debug APK and root-free emulator renderer must pass with bounded transcripts and exact cleanup. No React DOM/react-store/DOM globals, Tauri/Rust Core, Kotlin product lifecycle/cache/UI state machine, fallback/dual-write, production signing/release/store/iOS/physical-device claim, permission, VPN/TUN/Core/packet-flow/network effect, direct `dev` merge, or residual process/emulator/port |
