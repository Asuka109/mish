...
| CUT-02a Domain actor factory | active as existing `/root/cut02_domain_actors`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut02a/mihomo-web-client`; branch `codex/cut02a-domain-actor-factory`; source `codex/typescript-cutover@baa752af`; `gpt-5.6-luna/max`, Chinese, confirmation-only | CUT-02 integrated; CUT-03 production composition exposed the missing public actor-construction boundary | Add only a typed public XState actor factory/composition API under `packages/domain/**` plus deterministic tests so consumers do not import transitive `xstate`; preserve the existing machines/effect contracts and single lifecycle authority; system-boundary not applicable; no oRPC/UI/host effect, compatibility path, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
