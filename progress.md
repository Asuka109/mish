...
| CUT-06 Destructive retirement | active as `/root/cut06_destructive_retirement`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut06/mihomo-web-client`; branch `codex/cut06-destructive-retirement`; cumulative ledger source `codex/typescript-cutover@016cd96e`, implementation baseline `565a7b02`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance, final-only escalation | CUT-01 through CUT-05 integrated and accepted on one cumulative branch | Exclusive scope is the complete production denylist and every necessary root manifest/workspace/tsconfig/lock, script/workflow/CI, active documentation, package graph and test update; `poc/**` is read-only and may not enter production. Delete root/workspace Cargo manifests/lock/config/toolchain/lint files, `crates/**`, `mobile-core/**`, every production `*.rs`, both `apps/*/src-tauri/**`, Tauri dependencies/config/permissions, legacy `packages/rpc-client/**` and `packages/bridge-protocol/**`, custom lifecycle runtime, Rust parity, old-protocol/compatibility/migration fallback/dual-write/shadow paths, and all old build/release/test/CI references. Final frozen install, denylist/POC graph walk, no-Rust/Tauri tool invocation, Web/Electron/RN typecheck/tests/builds, transcript/privacy/replay, dual-ABI APK and credential-free DMG fixture gates must pass without release/signing/deploy, credentials, external writes, real permission/network/VPN/TUN/Core/system effects, direct `dev` merge, or residual process/mount/emulator/port |
