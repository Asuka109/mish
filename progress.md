...
dependency correction and P0b renderer dependency repair are integrated. P4
has resumed with the exact ReactDOM graph and verified offline Electron archive;
P5 delivered its bounded source/Gradle foundation as draft PR #544 and stopped
before APK claims on a verified direct RN toolchain dependency gap. P0c is
integrated and P5 has resumed to close bundle/APK/emulator evidence. Both host
...
| P0d Electron bundler dependency repair | active; `/root/p0d_electron_vite_dep`; `/Users/asuka/.codex/worktrees/wave3b-p0d/mihomo-web-client`; baseline `a7725f99` | `poc/electron/package.json`, `poc/pnpm-lock.yaml` | P0, P0b | Add exact direct `vite@8.2.1` for the P4 bundler API already consumed by the DMG fixture, refresh only the isolated lock, and prove frozen install plus all POC typechecks without transitive Vitest resolution or install-script relaxation |
...
| P4 Electron admission | functionally accepted pending P0d direct bundler graph; PR #546 / `903f72ab`; `/root/p4_electron_admission`; StrictMode race fixed, `.pnpm` scan removed, worker and coordinator each completed consecutive real mounted-DMG passes with bounded timeout cleanup and no residual process/mount | `poc/electron/**` excluding manifests | P0, P1, P0b, P0d | ESM Electron main/preload/renderer launches under sandbox/context isolation, performs oRPC MessagePort handshake, exits cleanly, proves the React renderer Store adapter, and produces a credential-free runnable DMG fixture without system effects |
| P5 React Native admission | active after P0c resume; draft PR #544 / `47670a73`; `/root/p5_rn_admission`; `/Users/asuka/.codex/worktrees/wave3b-p5/mihomo-web-client`; resume baseline `30a12011`; TS, 11 tests, static deny, transcript replay, and Kotlin/Gradle native compile already passed; bundle/APK/emulator evidence pending | `poc/rn/**` excluding manifests | P0, P2, P3, P0c | RN New Architecture/Hermes resolves oRPC ESM, Query/XState/Mish Store, WebSocket/AbortSignal/AsyncIterable, and Kotlin TurboModule; proves the Store adapter in the RN renderer; dual-ABI debug APK installs/launches in a root-free emulator fixture without VPN/Core/network effects |
