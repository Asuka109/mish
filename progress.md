...
| CUT-07 Final cumulative acceptance | active as `/root/cut07_final_acceptance` against read-only worktree `/Users/asuka/.codex/worktrees/typescript-cutover/mihomo-web-client` at `codex/typescript-cutover@d1ea4653`; `gpt-5.6-luna/max`, Chinese reporting, read-only confirmation acceptance | CUT-06 integrated | Verify one cumulative branch contains every packet and no partial runtime; run exact Web/Electron/RN renderer, transcript/privacy/replay, production exclusion, denylist graph, frozen lock and package/build/artifact checks. Produce a bounded evidence report only; do not mutate source, publish, deploy, sign, invoke real permission/network/VPN/TUN/Core/system effects, or merge to `dev` |
