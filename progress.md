...
| CUT-03 Query/Store/UI state | real Web consumer follow-up dispatching to existing `/root/cut03_query_store_ui`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut03/mihomo-web-client`; branch/PR `codex/cut03-query-store-ui` / #553; rebase/merge source `codex/typescript-cutover@77374dfb`; current package head `61bfbdcb`; `gpt-5.6-luna/max`, Chinese, confirmation-only; coordinator frozen install, UI-state typecheck/tests 20, oRPC 16, domain 15, scope review passed; superseded package-only lifecycle run cancelled before follow-up | CUT-01, CUT-02, and P6g integrated | Add only the three admitted Web `workspace:*` dependencies and lock importer, then replace old Web remote-resource/lifecycle projection in the cutover composition with real oRPC Query/XState consumers; Event Iterator only to Query/XState, Store only UI presentation, component transients local; prove stale generation/reconnect, bounded retry/chunks, cleanup failure/cancellation, unmount/remount, accessibility, and production graph; no helper-only claim, source-relative workspace import, host shell/effects, Zag, compatibility/fallback/dual-write, POC import, Rust parity, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
