...
| CUT-06 Destructive retirement | delivered as commit `2eafa49e` / PR #559 from `codex/cut06-destructive-retirement`; final-head CI pending before cumulative integration; worker and coordinator frozen/full `check:pr` passed, including Domain 17, oRPC 16, ui-state 20, Web 8, scripts 67, Electron 9, RN 8, production graph `reachable=48 entries=8`, POC byte/isolation check, deterministic credential-free DMG fixture, dual-ABI APK, root-free `RN_ADMISSION_OK`, and exact cleanup | CUT-01 through CUT-05 integrated and accepted on one cumulative branch | Deletes Cargo/Rust/Tauri, `crates/**`, `mobile-core/**`, old RPC/bridge/custom lifecycle/runtime/parity/compatibility/fallback/dual-write/shadow and retired build/release/CI paths; rewires the six Web surfaces to the admitted oRPC/XState/Query composition and rewrites active docs/CI. `poc/**` is unchanged and production-excluded. No release/signing/deploy, credentials, external write, real permission/network/VPN/TUN/Core/system effect, direct `dev` merge, or residual process/mount/emulator/port |
| CUT-07 Final cumulative acceptance | pending CUT-06 final-head CI and cumulative integration | CUT-06 | Read-only acceptance is the only gate allowed to propose the cumulative `dev` merge; no release/deploy |
