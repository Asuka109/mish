...
| CUT-02 Domain actors | active `/root/cut02_domain_actors`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut02/mihomo-web-client`; branch `codex/cut02-domain-actors`; cumulative source `f65a0785`, implementation baseline `ffa46346`; `gpt-5.6-luna/max`, Chinese, confirmation-only | CUT-01 integrated on cumulative branch | Exclusive scope: new `packages/domain/**`, actor tests, and exact XState `5.32.5` dependency updates to cumulative root `pnpm-lock.yaml` only; implement XState v5 Runtime/Core/Profile/Capture/VPN-TUN/Settings/Updater/RPC-session actors with authority/operation/revision guards, cancellation/joined finalizers, replacement/recovery, bounded effect transcripts and deterministic replay; no host effects, custom runner/kernel, second authority, POC imports, old protocol, fallback, parity, or merge to `dev`; PR target only `codex/typescript-cutover` |
