...
| CUT-09 Electron installed-window persistence | active as `/root/cut04_electron_host` in `/Users/asuka/.codex/worktrees/wave3c-cut09/mihomo-web-client`, branch `codex/cut09-electron-persistence`, from accepted cumulative source `codex/typescript-cutover@a9c97c02`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance | User installed CUT-08 DMG and observed the window flash then exit; source diagnosis found `handleRendererReady()` and admission failures unconditionally call `requestQuit()` | Own `apps/desktop/**` only. Separate ordinary installed lifecycle from explicit bounded acceptance mode: default launch must remain alive after renderer readiness/failure until the user closes the window, while the credential-free fixture retains deterministic auto-quit/cleanup. Add source/unit and real mounted-DMG persistent-window regression evidence without timing-only assertions; preserve transcript privacy/security and no POC/compatibility/runtime fallback. Require desktop/full gates, clean artifacts and no signing/notarization/publish/deploy, external write, real permission/network/VPN/TUN/Core/system effect, or `dev` merge |
