...
owned-emulator follow-up through PR #544 / `bc93b599`. Worker and coordinator
evidence now includes real Hermes bundles, a dual-ABI APK, and root-free
emulator renderer acceptance with exact port/identity cleanup; Fast PR,
Android Rust, and Android lifecycle gates passed. Both host
...
| P0b Electron renderer dependency repair | integrated; PR #543; `ede4df6e` | `poc/electron/package.json`, `poc/pnpm-lock.yaml` | P0, P3 | Exact direct `react-dom@19.2.7` and `@types/react-dom@19.2.3`; frozen offline install, all POC typechecks, 43 focused tests, dependency/isolation/format checks, Android gates, and Fast gate retry passed; no packager or install-script relaxation |
| P0c React Native host-toolchain dependency repair | integrated; PR #545; `30a12011` | `poc/rn/package.json`, `poc/pnpm-lock.yaml` | P0, P2, P3 | Exact RN 0.87 official-template/tool consumers: Babel core/runtime `7.29.7`, Community CLI and Android CLI `20.2.0`, RN Babel/Codegen/Gradle/Metro `0.87.0`, and Metro Babel transformer `0.87.0`; frozen offline install, all POC typechecks, 54 tests plus 2 existing skips, dependency/isolation/format checks, Android gates, and Fast retry passed; no iOS/Jest/template extras or install-script relaxation |
| P0d Electron bundler dependency repair | integrated; PR #547; `31c1e151` | `poc/electron/package.json`, `poc/pnpm-lock.yaml` | P0, P0b | Exact direct `vite@8.2.1`; coordinator frozen offline install, five POC typechecks, P1/P2/P3 tests, public Vite API/dependency graph and format checks passed; Fast and Android Rust passed, and the unrelated Gradle-plugin resolution failure in the Android lifecycle gate passed on failed-job retry |
| P0e React Native Metro transformer dependency repair | integrated; PR #548; `937a10c4` | `poc/rn/package.json`, `poc/pnpm-lock.yaml` | P0, P0c | Exact direct `@react-native/metro-babel-transformer@0.87.0` replaces the erroneous unscoped direct package; worker and coordinator frozen offline install, five POC typechecks, P1/P2/P3, public API resolution, isolation/format guards and all required CI passed |
...
| P6f generated-lock ownership correction | integrated; PR #550 / `b8a89c98`; coordinator admission script, 13 tests, targeted format/lint, Fast PR, Android Rust, and Android lifecycle passed; synced to cumulative branch at `3979c103` | CUT-00 review | Amend only the P6 admission document/checker/tests so CUT-01 through CUT-05 may update the one cumulative root `pnpm-lock.yaml`, with sequential/merge reconciliation and no independent production merge |
| P6g CUT-03 Web importer correction | integrated through PR #554 / `1be6fc7b`; synchronized to cumulative branch at `77374dfb`; coordinator admission checker, 20 tests, targeted format/lint, exact scope review, Fast PR 7m07s, Android Rust 5m47s, and Android lifecycle 16m32s passed | CUT-03 delivery exposed an internally inconsistent scope | CUT-03 may update only `apps/web/package.json` for exact `@mish/ui-state`, `@mish/orpc-client`, and `@mish/domain` `workspace:*` dependencies plus its already-authorized cumulative lock importer; CUT-06 retains root manifests/tsconfig/CI cleanup and every architecture/version/one-shot/no-real-effect constraint remains unchanged |
| CUT-01 Contract/session | integrated on cumulative branch through PR #551 / `ffa46346`; coordinator frozen install, contracts/oRPC typecheck, oRPC 16, legacy RPC 25, P1 POC 13, cutover admission, targeted format/lint, exact scope/lock review, Fast PR, and Android Rust passed; Android lifecycle attempt 1 failed before tests on transient APK install-write and the same-head attempt 2 passed in 17m12s | CUT-00 and P6f integrated | Contract-first oRPC session authority, WebSocket/Event Iterator/MessagePort policy, explicit Web/Electron/RN identities, bounded transcripts/privacy, and exact lock importer are present only on the cumulative branch; no merge to `dev` |
| CUT-02 Domain actors | integrated on cumulative branch through PR #552 / `da7c8237`; worker `/root/cut02_domain_actors`; final head `83278d99`; coordinator frozen install, domain typecheck/tests 15, P2 tests 10/typecheck, oRPC 16, legacy RPC 25, admission, targeted format/lint, scope/lock review, and protocol/recovery review passed; final-head Fast PR 4m31s, Android Rust 3m18s, and Android lifecycle 15m45s passed | CUT-01 integrated on cumulative branch | XState v5 Runtime/Core/Profile/Capture/VPN-TUN/Settings/Updater/RPC-session actors with unified effect correlation validation, fresh generation/reconnect attempts, compensation/recovery-only states, bounded transcripts, and deterministic replay exist only on `codex/typescript-cutover`; no host effect or direct `dev` merge |
| CUT-02a Domain actor factory | integrated on cumulative branch through commit `60eeada0` / PR #555 / merge `c562cfa9`; coordinator domain typecheck/tests 17, P2 tests 10/typecheck, oRPC 16, admission, declaration/import and exact 3-file scope review passed; Fast PR 6m40s, Android platform 5m32s, and Android lifecycle 27m18s passed | CUT-02 integrated; CUT-03 production composition exposed the missing public actor-construction boundary | Typed public `createDomainActor` and structural actor/snapshot/subscription types let consumers run existing XState actors without importing transitive `xstate`; stop rejects late completion and preserves one lifecycle authority; system-boundary not applicable; no lock, oRPC/UI/host effect, compatibility path, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
| CUT-02b Domain ESM consumer boundary | integrated on cumulative branch through commit `f5dbc4f0` / PR #556 / merge `4c7245bc`; exact 13-file domain-source scope; coordinator domain typecheck/tests 17, Web consumer typecheck, diff/specifier/scope review passed; worker frozen install, P2 tests 10/typecheck, oRPC 16, legacy RPC 25, admission, format/lint passed; Fast PR 6m57s, Android platform 6m13s, and Android lifecycle 18m40s passed | CUT-03 Web typecheck proved public domain source `.ts` specifiers require a forbidden consumer tsconfig relaxation | Relative `packages/domain/src/**` imports now use ESM `.js` specifiers with unchanged public types/runtime semantics and no consumer tsconfig relaxation; system-boundary not applicable; no package/lock/API/logic/host effect, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
| CUT-03 Query/Store/UI state | integrated on cumulative branch through commit `d3ef168f` / PR #553 / merge `87792557`; exact 17-file authorized scope and lock importer; coordinator ui-state typecheck/tests 20, Web typecheck/focused session tests 7, admission, diff/scope review passed; worker frozen install, domain 17, oRPC 16, Web full 657/build, admission/graph 32, format/lint passed; Fast PR 4m51s, Android platform 2m59s, and Android lifecycle 17m21s passed | CUT-01, CUT-02b, and P6g integrated | Real XState RPC-session actor/oRPC authority/Query/Store Web composition, ESM-safe ui-state exports, bounded Query/Event Iterator semantics, StrictMode retirement, stale cache rejection, cleanup observability, and localized accessible retry are present only on `codex/typescript-cutover`; Query owns remote cache, Store owns only presentation, and no helper shell, fallback/dual-write, host effect, or direct `dev` merge was added |
| CUT-04 Electron host | integrated on cumulative branch through commit `b45daacc` / PR #557 / merge `a30c59ec`; exact 20-file corrected scope; coordinator desktop typecheck, 9 tests, build, admission, security/scope review, original two consecutive mounted fixture runs, and corrected legacy-contract tests 39 passed; worker frozen install, Domain 17, Desktop 9, target scripts 44, full Web 657/scripts 279/check:pr, 529-file format, build/fixture and cleanup passed; final-head Fast PR 7m34s, Android platform 4m09s, and Android lifecycle 14m38s passed | CUT-01, CUT-02, and CUT-03 integrated | Electron 43.4.0 hardened main/preload/renderer uses official oRPC MessagePort, fresh session authority, sandbox/context isolation, a minimal typed contextBridge surface, bounded shutdown/transcripts, deterministic replay, and static `DomainTranscript`; transitional buildable package command shapes remain only for CUT-06 deletion. No Tauri/Rust runtime restoration, compatibility adapter, renderer token/raw Electron/IPC/MessagePort access, POC production import, second RPC protocol, credentials, release/signing/notarization, external network, VPN/TUN/Core, privileged settings, direct `dev` merge, or residual process/mount |
| CUT-05 RN host/native seam | integrated on cumulative branch through commit `928c3962` / PR #558 / merge `565a7b02`; exact 35-file scope; coordinator RN typecheck, 8 tests, admission/P5 static, architecture/scope/diff review and zero-artifact checks passed; worker RN/P5/package/Web gates, dual-ABI Hermes debug APK inspection, root-free emulator `RN_ADMISSION_OK`, and exact emulator/ADB/5558/APK/build cleanup passed; final-head Fast PR 6m27s and Android platform 4m11s passed; Android lifecycle attempt 1 failed after zero tests on transient test-APK `install-write`, while unchanged-head failed-job attempt 2 passed in 28m36s | CUT-01, CUT-02, CUT-03, and CUT-04 integrated | RN 0.87.0 New Architecture/Hermes composition consumes contracts/oRPC, XState actors, Query core, and framework-agnostic Store; Kotlin reports fixed capability facts only and the manifest requests no permission. No `src-tauri` edit, React DOM/react-store/DOM global, Tauri/Rust Core reuse, Kotlin product lifecycle/cache/UI state, fallback/dual-write, signing/release/store/iOS/physical-device claim, permission, VPN/TUN/Core/packet-flow/network effect, direct `dev` merge, or residual process/emulator/port |
| CUT-06 Destructive retirement | integrated on cumulative branch through commit `c14d43d8` / PR #559 / merge `d1ea4653`; worker and coordinator frozen/full `check:pr` passed, including Domain 17, oRPC 16, ui-state 20, Web 8, scripts 67, Electron 9, RN 8, production graph `reachable=48 entries=8`, POC byte/isolation check, deterministic credential-free DMG fixture, dual-ABI APK, root-free `RN_ADMISSION_OK`, and exact cleanup; final-head CI Product 1m02s, Electron 52s, RN 4m19s passed. Initial workflow heads failed before job creation because the checkout action SHA was truncated; the corrected 40-character pin and unconditional three-job checker passed | CUT-01 through CUT-05 integrated and accepted on one cumulative branch | Deletes Cargo/Rust/Tauri, `crates/**`, `mobile-core/**`, old RPC/bridge/custom lifecycle/runtime/parity/compatibility/fallback/dual-write/shadow and retired build/release/CI paths; rewires the six Web surfaces to the admitted oRPC/XState/Query composition and rewrites active docs/CI. `poc/**` is unchanged and production-excluded. No release/signing/deploy, credentials, external write, real permission/network/VPN/TUN/Core/system effect, direct `dev` merge, or residual process/mount/emulator/port |
| CUT-07 Final cumulative acceptance | accepted read-only at `codex/typescript-cutover@a9c97c02`; `/root/cut07_final_acceptance` verified full ancestry and unchanged POC, frozen root/POC installs, admission/retirement/POC/CI/format, `check:pr`, Domain 17, oRPC 16, ui-state 20, Web 8 plus unfiltered 69/69, scripts 67, Electron 9, RN 8, POC/replay, credential-free DMG, dual-ABI Hermes APK, root-free `RN_ADMISSION_OK`, and exact artifact/device/port/mount/process cleanup | CUT-07a integrated | Cumulative branch is ready only for final maintainer confirmation before merge to `dev`. Evidence is bounded/debug and makes no release, signing, notarization, deployment, physical-device, real permission/network/VPN/TUN/Core/system-effect claim; cumulative merge HEAD has inherited PR/ancestry evidence but no separate check-run |
| CUT-07a Retired Web test cleanup | integrated through commit `b402f8a5`, PR #560, and cumulative merge `a9c97c02`; unfiltered Web Vitest improved from 15 failed suites / 5 failed assertions / one collection error to 12 suites / 69 tests all passing; coordinator independently reproduced 69/69 and clean diff. Product 1m01s, Electron 45s, and RN 3m59s CI passed | CUT-07 exposed stale tests after CUT-06 retirement | Deleted only tests bound to removed route/provider/fixture/native-effect/schema surfaces and rewrote `window-startup.test.ts` for current DOM-only startup behavior. Preserved cutover session/pages plus model/presentation/i18n/UI primitive coverage; no production/POC/compatibility/host-effect/release/deploy/direct-`dev` change |
| CUT-08 Local artifact handoff | delivered from accepted cumulative source `codex/typescript-cutover@a9c97c02` by `/root/cut07_final_acceptance`; retained under ignored `.scratch/deliverables/typescript-cutover-a9c97c02` with SHA-256 manifest and verification report. DMG `cdfd8a47…b7f3f` (109,344,851 bytes); APK `dc21e501…52901` (70,651,699 bytes) | CUT-07 accepted | Production Electron fixture mounted/launched/exited 2/2 with 18 bounded events, zero active streams, cleanup, sandbox/context isolation and privacy checks; RN debug APK contains Hermes plus arm64-v8a/x86_64 libraries and owned root-free emulator returned `RN_ADMISSION_OK`. Coordinator independently verified both hashes, valid DMG checksum, APK ZIP/signature, ABI libraries and binary marker. Source/POC remained clean; ADB/5558/mount/process cleanup is zero. Artifacts are unsigned/debug only and were not signed, notarized, published or deployed; merged debug manifest metadata is recorded in `VERIFICATION.md` and no real permission/network/VPN/TUN/Core/system effect ran |
| CUT-09 Electron installed-window persistence | integrated through commits `533428ad`/`7ec90991`, PR #561, and cumulative merge `c1b78fc9`; exact seven-file `apps/desktop/**` scope. Desktop typecheck, 13 tests, build, full `check:pr`, admission/graph/POC/format/lint, explicit auto-quit fixture 2/2, and default persistent mounted-DMG 1/1 passed; Product 59s, Electron 1m00s, RN 4m13s CI passed. Coordinator independently reproduced typecheck/tests/build and mounted default persistence | User installed CUT-08 DMG and observed the window flash then exit; source diagnosis found `handleRendererReady()` and admission failures unconditionally call `requestQuit()` | Default ready/failure/timeout keeps the window and live session authority open until user close; fixture mode receives a typed disposition, waits for renderer-disposed acknowledgement, then performs bounded cleanup/quit. Fixture always rebuilds from current source, proves mode/default readiness and owned process identity, and leaves the user's pre-existing CUT-08 `/Volumes/Mish` mount untouched. No POC/compatibility/fallback, signing/notarization/publish/deploy, external write, real permission/network/VPN/TUN/Core/system effect, or `dev` merge |
| CUT-10 Replacement persistent DMG handoff | accepted from cumulative source `codex/typescript-cutover@c1b78fc9`; retained under ignored `.scratch/deliverables/typescript-cutover-c1b78fc9`. DMG SHA-256 `50b71436…e8f18f`, 109,341,778 bytes, with `SHA256SUMS` and `VERIFICATION.md` | CUT-09 integrated | Current-source production fixture default launch reached `mode=default`, renderer readiness, `keep-session`, and verified owned-process liveness before self-only SIGTERM user-close; clean 17-event transcript, zero active streams, cleanup one, security/privacy passed. Explicit auto-quit passed 2/2 with 19 events and cleanup. Coordinator independently verified hash and DMG checksum; source/temporary mounts/processes/ADB/5558 are clean, while the user's old read-only CUT-08 mount remains untouched. Old flashing DMG is superseded; prior Android APK remains accepted and unchanged. No signing/notarization/publish/deploy, external write, real permission/network/VPN/TUN/Core/system effect, or `dev` merge |
| CUT-11A Product UI restoration | integrated through commit `5771fe0c`, PR #563, and cumulative merge `8ec8c8c9`; exact `apps/web/**` scope. Web 12 files/72 tests, focused 11, typecheck, package entry, production build, admission/Tailwind/i18n/format passed; Product 54s, Electron 45s, RN 3m42s CI passed | User found the replacement DMG renders only the CUT-04 admission probe; audit also found CUT-06 replaced the prior product routes with a minimal read-only cutover shell | Restored the six-route Mish product hierarchy, responsive navigation, Query-backed loading/empty/error states, read-only capability language, local presentation interactions and formal `@mish/web`/styles exports. No deleted provider/custom RPC/Tauri/Rust/compatibility/fallback, fake commands/system effects, root lock, native/POC change, publish/deploy, real system effect, or `dev` merge |
| CUT-11B Electron truthful projections | integrated through commits `3bf81296`/`a1380966`, PR #562, and cumulative merge `92d95e72`; exact seven-file `apps/desktop/**` scope. Desktop 5 files/19 tests, typecheck/build, full `check:pr`, explicit fixture 2/2, persistent fixture 1/1 passed; Product 55s, Electron 53s, RN 4m04s CI passed | Current Electron host returned `accepted` without operation data; coordinator review also rejected treating session admission as Runtime-ready | Added one bounded typed projection authority for all six read operations, truthful empty user-data collections, owned read-only settings, correlation/session/stale/deadline/size/dispose validation and transcript results. Session-only admission is `degraded`; `ready` requires an explicit future Runtime observation. No renderer copy, fake user data, legacy provider/RPC/Tauri/Rust/compatibility, command/real system effect, publish/sign/deploy, or `dev` merge |
| CUT-11C Electron product-surface integration | integrated through commit `a87dfbae`, graph fix `e0c34a41`, PR #564 / head `dad06a05`, and cumulative merge `f6ae4056`. Desktop 19, Web 11, Domain 17, oRPC 16, ui-state 20, mobile 8, scripts 67, typecheck/build/format/lint, explicit mounted fixture 2/2 and persistent 1/1 passed; Product 1m07s, Electron 41s, RN 4m13s CI passed | CUT-11A and CUT-11B integrated | Formal `@mish/web` dependency mounts AppRoutes/styles inside the single Query/session composition; Electron source feeds `CutoverViewProvider`, StoreSurface is hidden/test-owned, and the bounded renderer report proves six visible routes, visible Status, and no visible Connected/remount placeholder. Production graph resolves the explicit apps/web workspace and remains `RETIREMENT_GRAPH_OK`. No UI copy, second actor/client/runtime, legacy/compatibility/Rust/Tauri, fake commands/data, real effects, release, or `dev` merge |
| CUT-12 Corrected product-UI DMG handoff | active as existing `/root/cut07_final_acceptance` from cumulative implementation source `codex/typescript-cutover@f6ae4056`, ledger-synchronized read-only head `9e24084f`; `gpt-5.6-luna/max`, Chinese reporting, evidence-only acceptance; owns ignored `.scratch/deliverables/typescript-cutover-f6ae4056` and disposable build/temp paths only | CUT-11A through CUT-11C integrated | Generate a replacement unsigned macOS arm64 DMG and retain SHA/report. Rebuild current source; verify mounted default launch persists after readiness, six-route product surface and Status degraded evidence are present, Connected/remount is not visible, and self-only user-close cleans the session. Run explicit auto-quit 2/2, DMG checksum, security/privacy/source-clean checks. Mark both earlier DMGs superseded. Do not edit tracked/progress/POC, detach user mounts, sign/notarize/publish/deploy, perform external/real network/VPN/TUN/Core/system effects, or merge `dev` |
