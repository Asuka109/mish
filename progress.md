...
| CUT-01 Contract/session | active `/root/cut01_orpc_contract_session`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut01/mihomo-web-client`; branch `codex/cut01-orpc-contract-session`; cumulative source `2bbfc634`, implementation baseline `3979c103`; `gpt-5.6-luna/max`, Chinese, confirmation-only | CUT-00 and P6f integrated | Exclusive production scope: `packages/contracts/**`, new `packages/orpc-client/**`, and bounded exact packet dependency updates to root `pnpm-lock.yaml`; implement contract-first oRPC session/WebSocket/Event Iterator/MessagePort policy with auth, exact version, generation, correlation, stale rejection, deadline, size bound, cancellation/cleanup, reconnect baseline, bounded transcripts/privacy; PR targets only `codex/typescript-cutover`, never `dev` |
