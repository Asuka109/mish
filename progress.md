...
| CUT-02 Domain actors | awaiting final-head CI on PR #552 / `83278d99`; worker `/root/cut02_domain_actors`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut02/mihomo-web-client`; branch `codex/cut02-domain-actors`; cumulative source `f65a0785`, implementation baseline `ffa46346`; `gpt-5.6-luna/max`, Chinese, confirmation-only; coordinator frozen install, domain typecheck/tests 15, P2 tests 10/typecheck, oRPC 16, legacy RPC 25, admission, targeted format/lint, scope/lock review, and protocol/recovery review passed | CUT-01 integrated on cumulative branch | Exclusive scope: new `packages/domain/**`, actor tests, and exact XState `5.32.5` dependency updates to cumulative root `pnpm-lock.yaml` only; XState v5 Runtime/Core/Profile/Capture/VPN-TUN/Settings/Updater/RPC-session actors have unified effect correlation validation, fresh generation/reconnect attempts, compensation/recovery-only states, bounded transcripts, and deterministic replay; no host effects, custom runner/kernel, second authority, POC imports, old protocol, fallback, parity, or merge to `dev`; merge only after all required CI passes on `83278d99`, and only to `codex/typescript-cutover` |
