...
| CUT-11C Electron product-surface integration | active as existing `/root/cut04_electron_host` on branch `codex/cut11c-electron-product-surface` from cumulative implementation source `codex/typescript-cutover@8ec8c8c9`; coordination ledger synchronized at `14536e41`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance; owns `apps/desktop/**` and exact root `pnpm-lock.yaml` importer only | CUT-11A and CUT-11B integrated | Add formal `@mish/web` dependency and mount its product AppRoutes/styles inside the existing Query/session composition. Feed the Electron authority into `CutoverViewProvider`; keep StoreSurface/admission reports hidden and test-owned, retain default persistence and explicit auto-quit cleanup. Fixture must assert visible six-route product DOM and absence of visible `Connected`/`remount`. No UI copy, cross-package relative import, legacy/compatibility/Rust/Tauri, fake commands/data, real effects, release, or `dev` merge |
