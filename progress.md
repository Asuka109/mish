...
| CUT-03 Query/Store/UI state | delivered through commit `d3ef168f` / PR #553 on existing `/root/cut03_query_store_ui`; exact 17-file authorized scope and lock importer; coordinator ui-state typecheck/tests 20, Web typecheck/focused session tests 7, admission, diff/scope review passed; worker frozen install, domain 17, oRPC 16, Web full 657/build, admission/graph 32, format/lint passed; required CI pending; not yet integrated | CUT-01, CUT-02b, and P6g integrated | Real XState RPC-session actor/oRPC authority/Query/Store Web composition, ESM-safe ui-state exports, bounded Query/Event Iterator semantics, StrictMode retirement, stale cache rejection, cleanup observability, and localized accessible retry are delivered without Web tsconfig relaxation or test polyfill. Preserve Query-only remote cache, Store-only presentation, one legacy Events consumer, and no helper shell, fallback/dual-write, host effect, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
