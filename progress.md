...
| CUT-07 Final cumulative acceptance | blocked read-only at `codex/typescript-cutover@d1ea4653`; `/root/cut07_final_acceptance` passed frozen install, admission/retirement/POC/CI graphs, `check:pr`, Web 8, Electron 9, RN 8, Domain 17, oRPC 16, ui-state 20, scripts 67, POC/replay, credential-free DMG fixture, dual-ABI APK, and root-free owned-emulator acceptance with exact cleanup; an additional unfiltered Web Vitest run failed 5/81 tests plus one collection error because retired tests still import deleted legacy capture/fixture/platform/browser-provider contracts | CUT-06 integrated | Preserve the positive cumulative evidence but do not propose or merge to `dev` until CUT-07a removes or rewrites only the stale retired tests and a fresh unfiltered Web suite plus all exact gates pass; no source mutation was made by CUT-07 |
| CUT-07a Retired Web test cleanup | active as `/root/cut06_destructive_retirement` in `/Users/asuka/.codex/worktrees/wave3c-cut07a/mihomo-web-client`, branch `codex/cut07a-retired-web-tests`, from cumulative source `codex/typescript-cutover@d1ea4653`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance | CUT-07 exposed stale tests after CUT-06 retirement | Own only stale Web test files/config needed to remove deleted legacy-contract imports; preserve admitted production behavior and meaningful oRPC/XState/Query/UI coverage. Require unfiltered Web Vitest 0 failures/collection errors, exact Web/Electron/RN and transcript/privacy/replay gates, frozen install, admission/retirement/POC/CI graphs, format/lint/diff checks, clean artifacts, and no POC/runtime/host-effect/release/deploy/direct-`dev` merge |
