...
| CUT-03 Query/Store/UI state | resuming on existing `/root/cut03_query_store_ui`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut03/mihomo-web-client`; branch/PR `codex/cut03-query-store-ui` / #553; implementation baseline includes CUT-02a merge `c562cfa9`; current committed package work includes `61bfbdcb`; `gpt-5.6-luna/max`, Chinese, confirmation-only | CUT-01, CUT-02a, and P6g integrated | Use the public domain factory for one real RPC-session actor; Query exclusively owns oRPC remote cache/freshness/invalidation and Event Iterator output enters Query or the actor only; Store owns presentation only. Do not create a second Events consumer/cache, machine-id placeholder, helper-only shell, relative source import, host effect, fallback/dual-write, direct `dev` merge, release, or real network/VPN/TUN/Core effect; prove stale/reconnect, bounded retry/chunks, cleanup/cancellation, remount, accessibility, and production graph |
