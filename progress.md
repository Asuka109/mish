...
| CUT-02a Domain actor factory | delivered as commit `60eeada0` / PR #555 by `/root/cut02_domain_actors`; worktree clean; base only `codex/typescript-cutover`; coordinator domain typecheck/tests 17, P2 tests 10/typecheck, oRPC 16, admission, declaration/import and exact 3-file scope review passed; required CI pending | CUT-02 integrated; CUT-03 production composition exposed the missing public actor-construction boundary | Typed public `createDomainActor` and structural actor/snapshot/subscription types let consumers run existing XState actors without importing transitive `xstate`; stop rejects late completion and preserves one lifecycle authority; system-boundary not applicable; no lock, oRPC/UI/host effect, compatibility path, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
| CUT-03 Query/Store/UI state | blocked on CUT-02a after existing `/root/cut03_query_store_ui` correctly removed the uncommitted fake integration and left its worktree clean; branch/PR `codex/cut03-query-store-ui` / #553; current committed head includes `61bfbdcb` and cumulative merge `e784f336`; `gpt-5.6-luna/max`, Chinese, confirmation-only; coordinator frozen install, UI-state typecheck/tests 20, oRPC 16, domain 15, scope review passed | CUT-01, CUT-02a, and P6g | Resume only after the public domain actor factory exists; then add the three admitted Web dependencies/lock importer and replace the old owned remote-resource/lifecycle projection with real oRPC Query/XState consumers, with no machine-id placeholder or duplicate Events consumer/cache authority; prove stale/reconnect, bounded retry/chunks, cleanup/cancellation, remount, accessibility, and production graph; no host effects, fallback/dual-write, direct `dev` merge, release, or real network/VPN/TUN/Core effect |
