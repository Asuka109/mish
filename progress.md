...
| CUT-03 Query/Store/UI state | active again on existing `/root/cut03_query_store_ui`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut03/mihomo-web-client`; branch/PR `codex/cut03-query-store-ui` / #553; retained uncommitted WIP; coordinator focused Web session tests 7/7 with Node experimental Web Storage disabled, ui-state 20/domain 17/oRPC 16 passed | CUT-01, CUT-02b, and P6g integrated | Finish the real actor/oRPC/Query/Store composition and convert ui-state relative source specifiers to ESM `.js` within CUT-03 scope; never relax Web tsconfig or add a test polyfill. Preserve one session actor, Query-only remote cache, Store-only presentation, no second Events consumer, helper shell, fallback/dual-write, host effect, direct `dev` merge, release, or real network/VPN/TUN/Core effect; prove stale/reconnect, bounded retry/chunks, cleanup/cancellation, StrictMode remount, accessibility, and production graph |
