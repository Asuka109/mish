...
| CUT-11A Product UI restoration | active as existing `/root/cut03_query_store_ui` on branch `codex/cut11a-product-ui` from implementation source `codex/typescript-cutover@c1b78fc9`; coordination ledger synchronized into the cumulative branch at `2ff5e386`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut03/mihomo-web-client`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance; exclusive `apps/web/**` scope | User found the replacement DMG renders only the CUT-04 admission probe; audit also found CUT-06 replaced the prior product routes with a minimal read-only cutover shell | Restore the complete Mish product visual hierarchy and all six routes on the current oRPC/XState/Query facade, with responsive navigation, loading/empty/error states and interaction-local state. Do not restore deleted legacy providers, custom RPC, Tauri/Rust, compatibility/fallback/fixtures as product data, or claim unavailable commands/system effects. Add regression coverage proving the production Web surface is not the admission placeholder. No root lock, desktop/native/POC/progress edit, publish/deploy, real system effect, or `dev` merge |
| CUT-11B Electron product renderer/projections | active as existing `/root/cut04_electron_host` on branch `codex/cut11b-electron-product` from implementation source `codex/typescript-cutover@c1b78fc9`; coordination ledger synchronized into the cumulative branch at `2ff5e386`; worktree `/Users/asuka/.codex/worktrees/wave3c-cut09/mihomo-web-client`; `gpt-5.6-luna/max`, Chinese reporting, confirmation-only acceptance; owns `apps/desktop/**` and the exact root `pnpm-lock.yaml` importer change only | CUT-11A supplies the reusable Web product surface; current Electron host returns `accepted` without operation data and its renderer is only an admission probe | Add one bounded TypeScript desktop projection authority behind the existing oRPC handler, return operation-correlated truthful data for all six read procedures, and mount the shared `@mish/web` product renderer while retaining the hardened preload/MessagePort/session lifecycle. Admission instrumentation must be hidden/test-owned and production must never render `remount`. Add transcript invocation/result coverage, invalid/stale/deadline/cleanup cases, product-surface fixture assertions and production-exclusion checks. Do not add fake user profiles/traffic, legacy provider/RPC/Tauri/Rust/compatibility, commands/system effects, publish/sign/notarize/deploy, external writes, real network/VPN/TUN/Core/system effects, or `dev` merge |
