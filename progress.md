...
| CUT-04 Electron host | correction active after PR #557 head `1b751968`; worker `/root/cut04_electron_host`; the 19-file delivery passed coordinator desktop typecheck, 9 tests, build, format, admission, scope/security review and two consecutive credential-free mounted fixture runs with zero residual process/mount; final-head Android platform passed, while Fast PR exposed three transitional desktop-package command-contract failures plus a false collision between the new production `SemanticTranscript` export name and the legacy SimulatedHost denylist; authorize only the existing 19 files plus `packages/domain/src/index.ts` to add a static `DomainTranscript` alias with no implementation/behavior change, replace the Electron import, and preserve harmless buildable package command shapes until CUT-06 deletes the retired Tauri checks; dynamic/computed export-name evasion, weakened tests/checkers, and integration remain forbidden | CUT-01, CUT-02, and CUT-03 integrated | Electron 43.4.0 hardened main/preload/renderer uses the official oRPC MessagePort server, fresh session authority, sandbox/context isolation, a minimal typed contextBridge surface, bounded shutdown/transcripts, and deterministic replay. The correction may not restore a Tauri/Rust runtime or compatibility adapter. No renderer token/raw Electron/IPC/MessagePort access, POC production import, second RPC protocol, credentials, release/signing/notarization, external network, VPN/TUN/Core, privileged settings, direct `dev` merge, or residual process/mount is allowed |
