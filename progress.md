...
| CUT-12 Corrected product-UI DMG handoff | rejected after installed screenshot evidence showed the formal product DOM rendered without its stylesheet. The `fa97abea` DMG and its earlier acceptance claim are invalid; CUT-13 visual-packaging repair is active on the cumulative branch | CUT-11A through CUT-11C integrated; PR #565 fixture install-flow repair integrated | Diagnose and repair the production Electron CSS resource chain, add fail-closed computed-style and installed screenshot acceptance, regenerate the unsigned arm64 DMG, and repeat installed persistence/auto-quit/checksum/security/privacy/source-clean evidence. DOM route visibility alone is explicitly insufficient. Earlier `a9c97c02`, `c1b78fc9`, and `fa97abea` DMGs are superseded. Do not detach user mounts, sign/notarize/publish/deploy, perform external/real network/VPN/TUN/Core/system effects, or merge implementation into `dev` |
