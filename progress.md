...
| P4 Electron admission | acceptance follow-up active on PR #546 / `47cf390e`; `/root/p4_electron_admission`; coordinator real mounted-DMG rerun timed out after 30s while 5 other tests passed; must fix deterministic renderer/oRPC/store quit, bounded timeout diagnostics/cleanup, and remove explicit `.pnpm` prefix scanning before reacceptance | `poc/electron/**` excluding manifests | P0, P1, P0b | ESM Electron main/preload/renderer launches under sandbox/context isolation, performs oRPC MessagePort handshake, exits cleanly, proves the React renderer Store adapter, and produces a credential-free runnable DMG fixture without system effects |
