...
fn assert_semantic_proxy_state(
    actual: &mish_runtime::NetworkServiceProxyState,
    expected: &mish_runtime::NetworkServiceProxyState,
) {
    assert_eq!(
        actual.auto_discovery_enabled,
        expected.auto_discovery_enabled
    );
    assert_eq!(actual.bypass_domains, expected.bypass_domains);
    for (actual, expected) in [
        (&actual.http, &expected.http),
        (&actual.https, &expected.https),
        (&actual.socks, &expected.socks),
    ] {
        assert_eq!(actual.authenticated, expected.authenticated);
    ...
    assert_eq!(actual.pac_enabled, expected.pac_enabled);
    if expected.pac_enabled {
        assert_eq!(actual.pac_url, expected.pac_url);
    }
...
async fn complete_typed_baselines_round_trip_through_the_real_application_capture_path() {
    for preset in [
        ...
        assert_semantic_proxy_state(&scenario.host.actual_proxy_state(), &baseline);
        assert_semantic_proxy_state(&scenario.host.observed_proxy_state(), &baseline);
        assert!(scenario.host.journal_snapshot().is_none());
    }
}

#[tokio::test]
async fn ordinary_system_proxy_transaction_has_no_authorization_or_helper_effects() {
