...
mock server for integration tests. Neither
...
and native integrations remain unimplemented. The isolated
[`crates/mihomo-controller/`](crates/mihomo-controller/) library implements the
pinned read-only Controller boundary but is not composed into the bridge or UI.
...
- A local Rust bridge service that owns the Mihomo core lifecycle and Mish
  application state, serves the offline web bundle, and exposes same-origin
