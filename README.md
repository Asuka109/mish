...
An open-source, cross-platform Mihomo client with a shared React and TypeScript
product layer. macOS is the first target. The same offline web bundle is intended
to run in a browser and inside a thin Tauri shell.

## Current status

The repository now contains a production TypeScript web foundation in
[`apps/web/`](apps/web/) and the retained interactive design reference in
[`sketch/`](sketch/). The production app exposes the six stable destinations and
uses a typed fixture adapter for the complete Status experience. Network values
and actions are still demo data; the local agent, RPC transport, Tauri shell,
Mihomo lifecycle, and native integrations have not been implemented.
...
- A local Rust agent that owns Mihomo lifecycle and application state, serves the
  offline web bundle, and exposes same-origin JSON-RPC over WebSocket.
...
validate the complete Part 1 surface with:
