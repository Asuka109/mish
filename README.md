...
pinned read-only Controller boundary; the desktop bridge can compose it when an
explicit Controller is supplied, while the current Tauri shell does not.
The Tauri macOS composition includes confirmed, reversible System Proxy
reconciliation through a narrow platform adapter. Controller-backed Tauri
Status, TUN, privileged helpers, status-bar behavior, signing, and native
material remain unimplemented.
...
The Routes destination is a fixture-backed policy-group workspace. It preserves
...
choice scoped to a Selector group. The desktop RPC adapter remains read-only and
disables route mutations until the bridge exposes a confirmed implementation.
