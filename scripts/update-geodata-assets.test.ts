...
test("downloads the exact required release assets and records deterministic provenance", async () => {
  ...
  const outputDirectory = path.join(temporary.path, "snapshots");
  const manifestPath = path.join(temporary.path, "manifest.json");
  ...
  for (const asset of manifest.assets) {
    const downloaded = readFileSync(path.join(outputDirectory, String(release.id), asset.name));
  ...
  assert.deepEqual(
    readdirSync(outputDirectory).filter((entry) => entry.includes("staging")),
    [],
  );
});
...
test("rejects missing and duplicate required release assets before downloading", async () => {
  ...
  const run = (candidate: GeodataRelease) =>
    updateGeodataSnapshot({
      ...
      manifestPath: path.join(temporary.path, "manifest.json"),
      outputDirectory: path.join(temporary.path, "snapshots"),
      release: candidate,
    });
...
test("size or digest mismatch leaves the prior manifest and published snapshots unchanged", async () => {
  ...
  assert.equal(readFileSync(manifestPath, "utf8"), "prior manifest\n");
  assert.equal(existsSync(path.join(outputDirectory, String(release.id))), false);
  assert.deepEqual(
...
test("rejects a same-size corrupted published snapshot without downloading over it", async () => {
  ...
  const geositePath = path.join(outputDirectory, String(release.id), "geosite.dat");
