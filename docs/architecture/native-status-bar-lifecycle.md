...
document startup placeholder. A synchronous self-hosted appearance bootstrap
runs under the production CSP before the reveal module, but the reveal does not
wait for image decoding, animation frames, desktop bridge bootstrap, or React's
first tree. A bounded `reveal_main_window` command applies the previously
computed login-launch policy, so the WebView cannot promote a background login
launch into a visible window.
