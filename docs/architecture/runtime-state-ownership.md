...
| Status capture-session baseline, totals, and sparkline                                              | `apps/web/src/pages/status-session.ts` stores `active`, cumulative baselines, previous source arrays, and output arrays in one React `useRef`; it truncates each output array to 60. `status-page.tsx` decides activity from the current client's capture DTO. | Reset occurs only when that mounted hook observes both capture modes inactive. A fresh mount takes the next received cumulative values as its own baseline and drops the source series seen at that instant. | Misclassified Web **Authority**. It has no session identity or revision. Remount loses totals/history; reconnect timing can move the baseline; two clients opened at different times disagree; a missed inactive snapshot can join two capture sessions; a Core/Profile replacement can make overlap matching append unrelated samples. | One Rust process-memory capture-session **Authority** with an atomic DTO. `P0`.                                                                 |
...
| Profile selected for the next launch                                                                | `apps/web/src/data/profile-provider.tsx` reads and writes `mish.selected-profile-id` in `window.localStorage`; `StatusPage` and capture commands consume it when Core is inactive.                                                                             | Per-browser-origin persistence. There is no shared revision or bridge publication, and the native status bar cannot observe it.                                                                              | Misclassified Web **Authority**. Desktop WebView and Browser Client can select different next-start Profiles; clearing origin data changes device intent; simultaneous clients race by sending different Profile IDs.                                                                                                                   | Move the preference behind the Rust Profile/settings Seam, publish its revision, and keep only a temporary Web **Optimistic projection**. `P1`. |
...
The current Status behavior is deterministic inside one mounted hook, but the
hook has no application-wide identity:
...
These are ownership failures, not evidence that all Status telemetry must move.
Memory, active-connection count, effective-rule count, and low-level Core
Traffic observations are already produced by Rust and can remain derived.
...
- A subscription response is an ordering barrier. The server installs the
  receiver, captures one atomic snapshot, returns it before later updates, and
  discards updates at or below that baseline revision for that subscription.
- A client accepts a reconnect baseline regardless of its cached revision,
  replaces cached authority when `authorityId` changes, and otherwise ignores
  updates whose revision is not greater than the accepted revision.
...
The migration is one end-to-end vertical slice:

1. Add the Rust Module, deterministic clock/source adapters for tests, Rust DTO,
   shared schema, and a new bridge protocol version.
2. Publish `recentTraffic` atomically in the Status baseline and updates.
   During one protocol version, retain the existing low-level `traffic` field
   unchanged for compatible clients and fixtures.
3. Switch Status rendering to `recentTraffic`. Delete
   `useStatusSessionTraffic`, its overlap heuristic, and its client baseline.
   The Web projection may convert paired samples to the existing sparkline
   props.
4. Make fixtures generate explicit authority/session/revision values; never
   synthesize production authority inside `RpcStatusClient`.
...
### Slice A — Rust-authoritative recent capture-session Traffic (`P0`)
...
**Delivery:** implement the complete compatibility/cutover sequence above in
one vertical slice, including desktop/browser simultaneous consumers and the
...
### Slice B — Rust-authoritative selected Profile intent (`P1`, after Slice A)
...
**Delivery:** move the next-launch selected Profile from
`mish.selected-profile-id` into a revisioned Rust preference; publish it to Web
and native consumers; use it for capture start; remove production localStorage
authority. Keep the inactive configured-route catalog as a Web **Bounded
cache** keyed by the confirmed selection revision.
...
- Web localStorage ownership of selected Profile intent;
