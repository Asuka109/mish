...
| Core lifecycle, aggregate launch, capture selection, System Proxy and TUN observation               | `MishRuntime`, `ProfileActivationCoordinator`, capture reconciliation Modules, and platform adapters publish `RuntimeStatus`, activation snapshots, protocol-26 Capture scope/operation identity, and native status-bar projections.                                                                                    | Rust serializes mutations and re-observes Core, managed listener, operating-system proxy state, helper state, routes, and DNS. Status subscriptions provide a current baseline; Web feedback consumes the published Capture `scopeEpoch/operationId`.                                             | **Authority** and **Derived DTOs**. React pending flags and selected button values are **Optimistic projections** only. Local operation identity never replaces aggregate Capture authority, and remount cannot change the active transaction.                                                          | Rust remains authoritative. Do not move button loaders, dialog state, or animation state. `retain`.                                                                                                                      |
| Android VPN lifecycle and observed platform resources                                               | Shared Rust `mish-vpn` lifecycle state owns product phase, operation/session, revision/sequence, cancellation, stale completion, and recovery policy. Kotlin owns VPN permission, foreground service, TUN/routes/DNS, validated underlying networks, protected sockets, Core effects, and bounded facts.                | A command settles only after the matching fact barrier or cleanup completion. Process replacement creates a new Rust authority and uses only a minimal foreground-expected recovery record; it never reconstructs or replays product state.                                                       | Rust **Authority** plus Kotlin **Derived DTOs** from current platform observation. React retains only the latest validated snapshot and pending presentation. Kotlin persistence is platform safety evidence, not a product cache.                                                                      | Keep this split. Running requires every same-session resource and one public request; stop/failure cleanup remains Pending until Mish-owned Core/TUN/network state is clean. `retain`.                                   |
