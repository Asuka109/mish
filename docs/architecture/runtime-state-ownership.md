...
| Latest validated RPC snapshots and connection-stale flags                                           | RPC clients and React providers retain the newest schema-validated snapshot and transport state. Protocol 25 gives complete Status, Profile, Events, and detailed Traffic snapshots one parent application authority/epoch/order accepted through a shared deep Module; stream-specific nested revisions remain intact. | Replaced only by an accepted subscription baseline after reconnect. Stale requests, commands, retired subscriptions, and older runtime epochs cannot publish.                                                                                                           | Web **Bounded cache** of authority, not a writer. A stale cache may be displayed as stale but cannot manufacture success. Equal parent order is idempotent only for equal deep content; conflicting content is rejected.            | Keep caches. The Status envelope composes with, and does not replace, recent-Traffic authority/session/revision. `observe`.                                                       |
