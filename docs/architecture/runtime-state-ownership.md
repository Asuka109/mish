...
| Configured route catalog shown while Core is inactive                                               | `apps/web/src/data/configured-route-catalog.ts` requests the accepted selected Profile's catalog and keeps one result in React state.                                                                                                                                                                                        | Rebuilt when the accepted Profile ID, selection revision, or effective fingerprint changes. Profile/Status staleness, missing selection, request failure, or live runtime groups clear it. Scoped cancellation and response identity validation prevent delayed results from restoring an older catalog.                                                                                           | Web **Bounded cache** of persisted Profile authority. Optimistic selection projections never authorize it; loss causes a reload only. It is never command authority.                                                                                                                                                                                                           | Keep Web-owned and bind invalidation to accepted Profile identity plus semantic revision. `retain`.                                                                                                                      |
...
| Detailed Traffic active connections, rules, close authority                                         | `mish_runtime::TrafficDataSnapshot` keeps the public rows/rules DTO unchanged. `traffic_source_machine.rs` owns only binding, live, replacing, ended, failed/reconciling, and retired source authority plus close-operation/effect correlation across Profile, runtime, capture, Controller session, and monotonic sequence. | Controller reconnect, capture/runtime context replacement, Controller loss, Core exit, and shutdown retire or reconcile the prior authority. Only a complete fresh Controller baseline restores command admission; RPC subscribe still returns a baseline before updates.                                                                                                                          | Rust **Authority** for source-session and command targeting, with rows/rules remaining **Derived DTOs** outside the machine. Duplicate operation identity is idempotent, and late or replaced completions are retired before reducer mutation. The Controller session remains distinct from aggregate capture even though capture identity participates in the internal scope. | Keep independent from recent capture-session Traffic and keep all presentation state in Web. `retain`.                                                                                                                   |
...
| Internal TUN Helper removal occurrences                                                             | `TunHelperRemovalOccurrenceStore` in `crates/desktop-bridge` keeps the last 16 closed, redacted removal occurrences in one private atomic file; the existing Settings/Helper, package, Capture, and network domains remain the only lifecycle and cleanup authorities.                                                       | The authenticated bridge assigns one operation ID and monotonic admitted revision, persists phase/typed failure/observation/cleanup before releasing its serialized lifecycle lock, and reconciles one interrupted active occurrence before restart retry admission. Failed records re-enter the existing `tun-helper.lifecycle` notification projection with the same identity.                   | Rust **Bounded diagnostic evidence**, not a maintenance snapshot, replay log, event bus, or command authority. Loss or invalid storage fails removal admission closed; a successful retry changes current Helper state but cannot mutate a retained failure occurrence.                                                                                                        | Keep domain-specific and private. Do not move it into the generic state-machine kernel, Notification Center storage, Events history, Web state, or privileged package authority. `retain`.                               |
...
confirmed Profile ID, selection revision, and effective fingerprint.
