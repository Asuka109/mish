...
| Profile selected for the next launch                                                                | `ProfileSelectionAuthority` in `crates/profile/src/selection.rs` atomically persists one validated Profile ID and monotonic revision; `ProfileSnapshot.selection` publishes it through the existing Profile RPC seam.                                  | On upgrade, an absent selection file migrates the manager's valid last-successful Profile before repository fallback. The shared Profile/Settings mutation authority queues selection commands and spans launch selection read through activation completion. Missing/deleted selections reconcile safely. | Rust persisted **Authority** plus temporary Web **Optimistic projection**. Web accepts only a greater selection revision, uses selection+revision compare-and-select for rollback, and cannot send a Profile ID through capture. Selection alone never activates Core. | Implemented in protocol version 25. Keep Profile activation ordered before capture apply, and compose the nested selection revision with any parent Profile snapshot epoch. `retain`. |
...
revision. First upgrade migrates a still-valid durable last-successful Profile
before deterministic repository fallback. The launch transaction holds the
shared mutation permit from confirmed-selection lookup through activation
completion, and a failed Web switch rolls back only when its selected ID and
