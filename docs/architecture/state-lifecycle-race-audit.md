...
This document audits lifecycle and transition correctness at baseline
`28a6e1d` for [Issue #207](https://github.com/Asuka109/mish/issues/207). It
covers Rust runtime modules, the bridge/RPC seam, native projections, React
providers and hooks, command state, cancellation, rollback, stale results,
shutdown, and cleanup.

This is a lifetime and transition taxonomy, not a second ownership taxonomy.
The current authority column below records repository evidence. Decisions about
moving globally visible state to Rust, including recent Status traffic samples
and Current Profile preference, belong to
[Issue #204](https://github.com/Asuka109/mish/issues/204). If #204 changes the
canonical ownership vocabulary, this document must adopt that vocabulary
without changing the scope definitions or creating a parallel model.
...
| Current Profile preference while capture is off | Cross-client application intent implemented as window/client-local      | [`ProfileProvider`](../../apps/web/src/data/profile-provider.tsx) stores `mish.selected-profile-id`; toolbar and next aggregate launch consume it | browser `localStorage`; resets per origin/client storage, not per process                                                  | Profile ID only; immediate local change with manual rollback on a failed running switch                                                         | each client                                                                                                         | Global product intent is implemented per client. Authority placement belongs to #204; #207 requires operation identity and rollback rules           |
...
| Recent Status traffic series and session totals | Capture/activation session implemented per client                       | [`useStatusSessionTraffic`](../../apps/web/src/pages/status-session.ts); Status page                                                              | React ref; reset on local inactive observation or remount                                                                  | series-overlap heuristic; no capture-session ID                                                                                                 | hook unmount                                                                                                        | Global session telemetry is per client; contract and migration belong to #204                                                                       |
...
| P2       | Two clients select different Current Profiles while capture is off, then either launches                                 | no shared preference; each uses local storage                                 | Authority placement belongs to #204; #207 requires launch to record the selected Profile in its operation scope               |
...
- Current Profile selection while capture is running is optimistic and rolls
  back locally, but lacks cross-client authority and operation identity.
  Authority placement is #204; the transition contract remains #207 work.
...
These are independent vertical slices, not a repository-wide migration.
Issue #204 is a terminology and authority-placement dependency, not an excuse
to duplicate its recent-traffic contract.
...
- **Dependency:** harmonize field names with #204 if it lands first.
...
- **Acceptance:** hands-on only if visible timing or control feedback changes;
  otherwise confirmation-only.
...
- **Verification:** browser/status-bar simultaneous commands, Stop/Quit during
  ...
  second client.
- **Acceptance:** hands-on only for native menu parity; machine checks cover
...
- #204 terminology and cross-links are harmonized at the delivery checkpoint;
