...
- notification claim/lease/ack, baseline/update revision handling, and stale
  session rejection in
  [`notifications.rs`](../../crates/runtime/src/notifications.rs),
  [`bridge_protocol.rs`](../../crates/desktop-bridge/tests/bridge_protocol.rs),
  and [`rpc-notification-client.test.ts`](../../apps/web/src/data/rpc-notification-client.test.ts);
...
| Settings snapshot                               | Installed-application persisted plus process-scoped derived observations           | `SettingsService`; Desktop/Browser RPC clients, Web Settings provider, and native projections                                                | preferences persist in private `settings.json`; derived observations and Rust authority reset on process replacement                                       | Rust authority ID + complete-snapshot order; nested preference revision advances only on durable mutation                                                                     | Settings service publication, transport subscription retirement, and provider unsubscribe                       | Resolved in protocol 35: replacement baseline accepts a lower preference revision, while late retired transport/authority snapshots and equal-order conflicts are rejected                                                  |
...
| Activation lifecycle                            | Active Profile/Core plus command                                                   | [`ProfileActivationCoordinator`](../../crates/desktop-bridge/src/profile_activation.rs); RPC, startup, status bar, Product/Profile providers | process memory plus manager’s durable last-success evidence; reset by new command or shutdown                                                              | kernel correlation binds authority/scope/command/revision/effect plus Profile revision/fingerprint and committed runtime authority; running Capture shares the command prefix | kernel owns and joins effects/finalizers; shutdown cancels the exact command and waits for terminal observation | Resolved: commit requires authoritative runtime plus Capture confirmation; exact prior runtime/Capture compensation is confirmed or becomes RecoveryRequired                                                                |
| Core lifecycle effect                           | One admitted Profile activation/stop/recovery effect                               | `ProfileActivationCoordinator` through `MishRuntime`; process/mobile adapters execute opaque scoped commands                                 | one owned effect; superseded by newer scope/revision/effect and cleared only by exact finalization                                                         | machine authority + scope epoch + operation ID + admitted revision + effect identity; terminal publication additionally requires authoritative Core observation               | Profile kernel finalizer and adapter-owned process/TUN cleanup                                                  | Resolved: no public bare Core command; stale or replaced completion cannot publish success or clear current authority                                                                                                       |
...
| Traffic snapshot                                | Active Profile/Core observation session                                            | Rust Traffic source machine plus unchanged row/rule mapper; Traffic provider and native summary                                              | process memory; binding/live/replacing/ended/failed-reconciling/retired authority resets on Profile/runtime/capture/session replacement, loss, or shutdown | machine authority + Profile/runtime/capture/Controller session + monotonic source sequence                                                                                    | bounded runner owns close effects and retirement; provider clears pause/history on stale connection             | Resolved: a gap requires a complete baseline, and old-session observations or completions are retired rather than replacing current authority                                                                               |
...
| Notification center                             | Process-global plus one leased presentation session                                | deep Rust Notification Module; Web center/toast and native producers                                                                         | process memory, 128 records; preserved across runtime replacement; a lease resets on expiry, unsubscribe, socket loss, or client replacement               | monotonic snapshot and record revisions, stable IDs, dedupe keys, client/session identity, and lease generation                                                               | producer retirement, bounded retention, Rust lease requeue, client unsubscribe                                  | Creation, read, producer resolution, action execution, and record removal remain independent; only exact current lease ACK may fold presentation                                                                            |
...
| Updater Check                                   | Process-global updater authority plus one operation                                | `UpdaterService` Check reducer/runtime; desktop bridge and Web updater clients                                                               | process memory plus private accepted-metadata/candidate store; reset by a later admitted Check or service shutdown                                         | machine authority + scope epoch + operation ID + admitted revision + effect ID                                                                                                | bounded inbox owns effect tasks; cancel/finalizer and bounded shutdown join/retire every task                   | Resolved for Check: explicit CommittingAvailable cutoff prevents cancellation or a retired completion from erasing a committed candidate                                                                                    |
| Updater Continuation                            | Process-global updater authority plus one admitted candidate operation             | `UpdaterService` Continuation reducer/runtime; private HTTP, verification, and candidate-store adapters                                      | process memory plus bounded `recovery.json`, private partial, and immutable candidate; reset by verified replacement or retirement                         | machine authority + scope epoch + operation ID + admitted revision + effect ID + monotonic progress sequence                                                                  | bounded inbox owns download/verify/commit/finalizer tasks; restart recovery only observes and re-verifies       | Resolved: durable partial progress and immutable candidate publication are separate commit points; unknown candidate-commit outcomes are not replayed                                                                       |
| Traffic close commands                          | Active Profile/Core observation session plus command                               | Traffic source machine/effect adapter and runtime host; Traffic provider                                                                     | bounded process-memory operation ledger; no persistence                                                                                                    | machine authority + scope epoch + operation/effect ID + admitted revision + Profile/runtime/capture/session/sequence authority + bounded target IDs                           | runner cancellation/finalizer, source retirement, runtime-host replacement check, exact-operation Web cleanup   | Resolved: duplicate identity is idempotent; every mutation and terminal completion freshly revalidates current source scope, while stale, replaced, cancelled, panicked, or late completion cannot commit current authority |
...
| P0       | Settings process A reaches preference revision 8; process B restarts at revision 1; late A notification arrives after B  | Protocol 35 Settings Rust authority plus complete-snapshot order and transport generation       | Resolved: B's reconnect baseline replaces A, equal-revision observations advance independently, and retired A cannot reopen         |
...
| P1       | Updater Check A is cancelled or retired; Check B is admitted; a late A effect completes                                  | Check reducer validates authority, scope epoch, operation, admitted revision, and effect ID     | Resolved: the late completion is retired unchanged; an owned finalizer deterministically terminates any still-current failed task   |
...
- Settings does not optimistically mutate the snapshot. Complete snapshots use
  Rust authority/order acceptance, while the nested preference revision remains
  durable mutation evidence rather than derived-observation order.
...
- Current Profile selection is a client-local **Optimistic projection** of
  ...
  selection. Activation without running Capture remains separate. A running
  Capture Profile/backend switch is one Runtime-coordinator saga and commits
  only after Capture is Applied against the new authoritative runtime.
...
| Profile/Core activation         | data-bearing Rust State/Input/Effect reducer plus exhaustive DTO Adapter and transactional manager | Conforming: retain the public DTO and use the repository kernel for bounded effects, finalizers, and compensation    | command scope/revision, Profile revision/fingerprint, runtime authority, Capture correlation, panic/abort recovery, rollback, retry, and shutdown are structurally constrained |
...
| Updater Continuation            | data-bearing Rust Download/Verify/Commit/Recovery reducer plus bounded effect owner                | Conforming: retain the public updater DTO while the domain machine owns candidate lifecycle and recovery vocabulary  | durable partial and candidate commit points, six-part effect correlation, finalizing cleanup, restart re-verification, and failure-injection tests                             |
...
- **Delivery:** the typed DTO foundation was completed by Issue #236; Issue #288
  ...
  boundaries. Every command carries its target Profile revision/fingerprint,
  authority scope, monotonic revision, and effect correlation. Success requires
  the committed runtime authority and, for a running Capture switch, the
  requested Capture state observed as Applied against that runtime. One
  exhaustive Adapter retains the public DTO fields, and shared refinements
  reject invalid optional-field combinations.
...
- **Problem:** one DTO with optional fields represents states that should be
  ...
  identity.
- **Solution:** use a pure Rust reducer with kernel-owned bounded effects and
  finalizers. Retire stale completions by full correlation; confirm the exact
  prior runtime/Capture state on failure, otherwise publish Recovery Required.
  Retain one exhaustive DTO projection and the existing public field shape.
...
- **Cutover:** internal first; serialize golden snapshots before changing any
  transport shape.
- **Verification:** legal-transition/model tests, invalid-combination contract
  tests, duplicate/replacement completion, cancellation, task abort/panic,
  finalizer recovery, Capture correlation, compensation success/failure,
  runtime handoff barriers, retry, shutdown, and aggregate launch integration.
- **Acceptance:** confirmation-only.
