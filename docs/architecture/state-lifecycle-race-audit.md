...
| P2       | Two clients select different Current Profiles while capture is off, then either launches                                 | no shared preference; each uses local storage                                 | The ownership contract owns placement; launch must record the confirmed selected Profile revision in its operation scope      |
...
- Current Profile selection while capture is running is an **Optimistic
  projection** and rolls back locally, but lacks cross-client authority and
  operation identity. The ownership contract owns placement; the transition
  contract remains lifecycle work.
