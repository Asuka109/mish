...
| Current Profile preference while capture is off | Installed-application persisted Rust authority with a window/client optimistic projection | `ProfileSelectionAuthority`; Profile snapshots, toolbar, native launch, and next aggregate launch consume it                                 | private `selected-profile.json`; reconciles on missing/deleted Profiles and survives process, client, and RPC restart      | monotonic persisted selection revision; queued mutation permit; greater-revision acceptance; temporary client operation identity                | Profile service persists/reconciles; Profile provider clears only its matching optimistic operation                 | Delivered in protocol version 25. A future parent Profile snapshot epoch composes with, and does not replace, the nested selection revision         |
...
| P2       | Two clients select different Current Profiles while capture is off, then either launches                                 | Rust queues both commands and publishes revisions; launch reads confirmed selection | Preserve the nested selection revision when adding a parent Profile epoch; clients and capture commands never supply competing authority |
