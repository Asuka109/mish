...
- **React** owns composition and ephemeral interaction or display state. Search,
  ...
  cross-platform.
- The installed-mobile React `MobileShell` is the sole owner of persistent
  product chrome, top-level destinations, child routes, history/Back, product
  overlays, and DOM focus. Native code does not add a parallel persistent
  navigation owner.
...
| `S01` | Core phase, start, stop, and health                                                                                                                      | [`MishRuntime`](../../crates/runtime/src/lib.rs) owns typed lifecycle semantics and the narrow [`PrivilegedCoreHost`](../../crates/runtime/src/privileged_core.rs) effect port; [`DesktopMihomoProcess`](../../crates/desktop-bridge/src/managed_process.rs) consumes it while the desktop host injects the macOS implementation.                                 | `runtime-scoped`                 | Shared Rust owns phase and command completion; the desktop process or Mobile Core adapter owns engine facts and cleanup.                                                                                                                                      | Desktop executable, PID, signal, and loopback-listener assumptions must not enter the mobile service.                                                                                                              |
...
| `P03` | Profile activation, replacement, rollback, last-successful resume target, and safe stop                                                                  | Rust [`ProfileActivationCoordinator`](../../crates/desktop-bridge/src/profile_activation.rs) owns the kernel reducer/finalizers; the activation manager and Capture reconciler supply effects.                                                                                                                                                                    | `runtime-scoped`                 | Shared Rust application service; correlate Profile revision, runtime authority, and Capture operation, then cancel/join/finalize or confirm exact compensation before terminal publication.                                                                   | The product-neutral saga now has one authority, but remains composed in desktop bridge; a mobile composition must reuse it rather than duplicate it.                                                               |
...
| `T01` | Detailed active connections, rules, source identity, ordering, and bounded process-icon bytes                                                            | Shared Rust [`TrafficDataSnapshot`](../../crates/runtime/src/traffic.rs) and [`ProcessIcon`](../../crates/runtime/src/process_icon.rs) contracts plus desktop Controller source and runtime host; the desktop host injects the macOS resolver into Bridge.                                                                                                        | `capture/Traffic session-scoped` | Shared Rust owns session/sequence acceptance and icon byte/type bounds; the source owns data privacy, while the platform resolver owns filesystem validation and its bounded cache.                                                                           | Retain contracts; add a native source or resolver rather than copying Controller payloads or macOS icon APIs into Kotlin.                                                                                          |
...
| `M02` | VPN consent, notification permission, foreground service, TUN/routes/DNS, validated underlying networks, socket protection, and revoke/destroy callbacks | Kotlin [`MishVpnPlugin`](../../apps/mobile/src-tauri/plugins/mish-vpn/android/src/main/java/com/asuka109/mish/vpn/MishVpnPlugin.kt), [`MishVpnService`](../../apps/mobile/src-tauri/plugins/mish-vpn/android/src/main/java/com/asuka109/mish/vpn/MishVpnService.kt), and the closed JNI bridge.                                                                   | `platform-scoped`                | Android adapter serializes effects and carries machine authority, scope epoch, operation ID, admitted revision, and effect identity through Service cleanup; stale stop authority is ignored before resource mutation, and the notification cannot mint Stop. | Delivered for the first VPN slice. The generated Android facts contract and bounded latest-authoritative ingress prevent schema drift and terminal observation loss without giving Kotlin product-state authority. |
| `M03` | Embedded engine lifecycle, config validation/load, snapshots, closed commands, event poll, and returned buffers                                          | Versioned [`Mobile Core ABI v1`](mobile-core-abi.md) and Go wrapper; JNI executes identity, bounded validate/load, scoped start/stop, and runtime inspection calls.                                                                                                                                                                                               | `runtime-scoped`                 | Mobile Core validates lifecycle authority and rejects foreign or stale scope/revision before mutation; Shared Rust remains the issuer. Every ABI buffer is freed exactly once.                                                                                | Validation/load and one TUN-backed Core session are delivered. Routes, Traffic, general commands, and event-poll product wiring remain outside this slice.                                                         |
...
8. **Universal-view pressure.** Shared route components do not imply identical
   ...
   mobile composition.
9. **Persistent-shell regression.** Native platform adapters must not reintroduce
   tabs, bars, drawers, route selection, Back, product overlays, or focus state
   beside the Web `MobileShell`. Native authority remains limited to genuine
   typed platform effects such as VPN/TUN/Core and operating-system lifecycle.
...
Status: implemented for the bounded configuration authority and lifecycle
slice. ARM64 emulator/Appium evidence exists; physical ARM64 and x86_64 emulator
runtime acceptance remain required before the slice is accepted.
