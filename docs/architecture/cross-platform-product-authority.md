...
| `V02` | Installed-mobile top-level shell selection and Native-to-Web entry                                                                                       | Production-disabled Shared Rust [`MobileShellAuthority`](../../crates/mobile-shell/src/lib.rs) accepts only Android/Apple chrome destinations or validated platform deep links and emits one-way directives.                                                                                    | `process-global`                 | Shared Rust owns the selected top-level snapshot, monotonic revision, and bounded intent retirement; native adapters own platform input/delivery and React never writes back.                        | Contract delivered but not linked into `apps/mobile`; Android and Apple cutovers remain separate work. See [`mobile-native-shell-entry.md`](mobile-native-shell-entry.md). |
