...
| `D01` | Guided diagnostic run, checks, history, cancellation, and runtime replacement                                                         | Shared Rust diagnostic DTOs and Rust desktop [`DiagnosticCoordinator`](../../crates/desktop-bridge/src/diagnostics.rs); React keeps only display and command feedback.                                                                                                                    | `runtime-scoped`                 | Shared Rust service owns run identity, fixed probe policy, redaction, cancellation, and bounded history; platform adapters execute platform probes.                                                                      | Service implementation is still desktop-bridge-owned and mobile currently returns fixtures.                                                                  |
...
| `N01` | Semantic notification identity, dedupe, severity, revision, retention, read/resolved state, and action references                     | Shared Rust [`NotificationCenter`](../../crates/runtime/src/notifications.rs).                                                                                                                                                                                                            | `process-global`                 | Shared Rust; bounded retirement and application shutdown clear in-process records.                                                                                                                                       | Retain. Mobile must project the same semantic publications rather than invent Kotlin product messages.                                                       |
| `N02` | Localized notification copy, toast visibility, animation, and action-button pending display                                           | TypeScript registry and React notification delivery/adapter modules.                                                                                                                                                                                                                      | `view-local`                     | React; unmount and adapter disposal remove presentation, while Shared Rust retains semantic truth.                                                                                                                       | Intentionally remains React-owned. Android foreground-service notification is a separate platform obligation, not this product center.                       |
...
| `M01` | Android VPN product phase, message, session, and sequence                                                                             | Kotlin [`MishVpnStateMachine`](../../apps/mobile/src-tauri/plugins/mish-vpn/android/src/main/java/com/asuka109/mish/vpn/MishVpnStateMachine.kt) and [`MishVpnStateStore`](../../apps/mobile/src-tauri/plugins/mish-vpn/android/src/main/java/com/asuka109/mish/vpn/MishVpnStateStore.kt). | `runtime-scoped`                 | Shared Rust owns product transitions, operation/session identity, revision/sequence, redacted failure, and recovery policy.                                                                                              | Current prototype is duplicate product authority in Kotlin. It must not become production authority.                                                         |
...
| `M03` | Embedded engine lifecycle, config validation/load, snapshots, closed commands, event poll, and returned buffers                       | Versioned [`Mobile Core ABI v1`](mobile-core-abi.md) and Go wrapper; current JNI uses only ABI/version probes.                                                                                                                                                                            | `runtime-scoped`                 | Mobile Core adapter executes the ABI; Shared Rust owns application operation identity, preconditions, policy, and cleanup decision. Every ABI buffer is freed and every runtime is stopped before replacement.           | The ABI is an engine boundary, not a product service. TUN/config/lifecycle wiring is not delivered.                                                          |
...
| `M05` | Mobile VPN client command completion and snapshot acceptance                                                                          | React [`MobileVpnFixtureClient`](../../apps/web/src/platform/mobile-vpn-client.ts) accepts sequence only; the service command returns before the executor publishes its terminal event.                                                                                                   | `view-local`                     | React keeps pending display only; Shared Rust command results carry operation identity and terminal/unknown outcome, and snapshot acceptance compares authority/session plus sequence.                                   | Current immediate result can be the old snapshot, and a new session is not part of client acceptance. This is an optimistic-state and reconnect risk.        |
| `M06` | Persisted Android fixture lifecycle recovery record                                                                                   | Kotlin [`MishVpnStateStore`](../../apps/mobile/src-tauri/plugins/mish-vpn/android/src/main/java/com/asuka109/mish/vpn/MishVpnStateStore.kt) writes the complete product snapshot to `SharedPreferences` and repairs active-looking state after process start.                             | `durable installation state`     | No production authority is required for this prototype record; VS-A may delete it or replace it with the minimum safety evidence needed to recover one platform resource, then Shared Rust publishes safe stopped state. | The record conflates runtime truth with durable recovery. It is not a compatibility promise and cannot prove a service, TUN, or Core survived process death. |
...
1. **Duplicate Android authority.** Kotlin currently decides product phases,
   session creation, messages, sequence increments, persistence, recovery, and
   terminal transitions. Shared Rust already owns equivalent desktop semantics.
   Adding Profile, Routes, Traffic, Events, or Diagnostics directly to the
   service would fork product rules.
2. **Contract triplication.** The mobile VPN snapshot is separately declared in
   Kotlin, Rust plugin models, and TypeScript schemas. Until Shared Rust emits
   the canonical snapshot and generated/boundary tests constrain projections,
   fields and legal states can drift.
3. **Optimistic command completion.** `startFixtureLifecycle` and `stop` enqueue
   service work, then immediately read storage. React can clear its pending flag
   against an older snapshot before the terminal event arrives. Production
   commands require operation identity and a terminal or explicitly unknown
   outcome.
4. **Incomplete mobile ordering.** `MobileVpnFixtureClient` rejects by sequence
   alone. Cross-process or replacement authority needs authority/session plus
   sequence, with a full baseline before later events.
...
6. **Fixture promotion risk.** Mobile currently relabels TypeScript fixtures as
   ...
   domain at a time.
7. **Cleanup gaps at the future JNI/TUN seam.** The current JNI code only probes
   version identity. No code yet proves descriptor closure, socket protection,
   Core stop/join, buffer release, or post-destruction reconciliation as one
