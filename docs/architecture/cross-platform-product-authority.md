...
| `M05` | Mobile VPN client command completion and snapshot acceptance                                                                          | React [`MobileVpnFixtureClient`](../../apps/web/src/platform/mobile-vpn-client.ts) accepts session plus sequence and configuration loads return operation-keyed terminal/unknown results; fixture lifecycle commands still return before service terminal events.                         | `view-local`                     | React keeps pending display only; Shared Rust command results carry operation identity and terminal/unknown outcome, and snapshot acceptance compares authority/session plus sequence.                                   | Configuration ordering is delivered; fixture VPN lifecycle completion retains the older optimistic completion risk.                                          |
...
3. **Optimistic command completion.** `startFixtureLifecycle` and `stop` enqueue
   ...
   outcome.
4. **Incomplete mobile ordering outside configuration loading.**
   `MobileVpnFixtureClient` now retires prior sessions and compares session plus
   sequence, while future VPN activation still needs the same full-baseline
   rule inside its Shared Rust authority.
