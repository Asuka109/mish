...
CUT-03's only additional production manifest is `apps/web/package.json`, the
real Web production consumer. It may declare only production `dependencies`
for `@mish/ui-state`, `@mish/orpc-client`, and `@mish/domain`, each using the
exact `workspace:*` specifier. No root `package.json`, root/apps Web tsconfig
(`tsconfig.json` or `apps/web/tsconfig*.json`), `pnpm-workspace.yaml`,
CI/scripts except `scripts/check-typescript-cutover-admission.ts`, or any other
app/package manifest is authorized. Those paths remain CUT-06 final root
manifest/tsconfig/CI cleanup or later host packet ownership. The existing
cumulative root `pnpm-lock.yaml` permission remains unchanged. This importer
correction does not change exact library pins, Query/XState/Store
responsibilities, dependency waves, the one-shot merge barrier, the direct
`dev` merge prohibition, or the no release/deploy/real network/VPN/TUN/Core/
...
| CUT-03 Query/Store/UI state        | `packages/ui-state/**`, `apps/web/src/**` UI/data projection files, UI tests, the only additional production manifest `apps/web/package.json` for the real Web production consumer with only production `dependencies` entries `@mish/ui-state`, `@mish/orpc-client`, and `@mish/domain`, each using exact `workspace:*`, and bounded generated updates to the cumulative root `pnpm-lock.yaml` for exact packet dependencies only; no host shell | CUT-01, CUT-02                     | Migrate remote state to Query, lifecycle to actors, UI-only state to the pinned Store wrapper, and Event Iterator sinks to Query/XState. Web renderer journeys, unmount/remount, stale/reconnect, accessibility, and production-graph checks pass; no DOM adapter in RN/shared code.                                                 |
...
- [x] CUT-03 Web importer is limited to `apps/web/package.json` and the three exact `workspace:*` dependencies; root manifests/tsconfig/workspace/CI and other manifests remain excluded.
