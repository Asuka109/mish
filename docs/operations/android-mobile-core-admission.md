...
| Expected signer        | exact lower-case SHA-256 certificate fingerprint from the source manifest                                               |
...
- complete pinned manifest, supported ABI, exact artifact digest, and the
  expected synthetic signer fingerprint admit;
...
signature-key, or network acceptance. The checked-in signer policy is a bounded
synthetic debug policy; it is not a real release/store certificate identity.
