...
const adbRun = (args: string[], allowFailure = false): string => {
  ...
  if (!allowFailure && result.status !== 0) {
    throw new Error(`adb ${args.join(" ")} failed: ${result.stderr ?? result.error?.message ?? "unknown error"}`);
  }
...
if (!serial) {
  ...
  serial = configuredPortRaw ? availableSerials.find((candidate) => candidate === expectedSerial) : availableSerials[0];
}
